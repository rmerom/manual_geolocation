...
function initialize() {
  ...
  initAutocomplete();
  $('#locateButton').click(locateByAddress);
}
...
function initAutocomplete() {
  ...
  var text = $('#location').get()[0];
  var autocomplete = new google.maps.places.Autocomplete(text, {types: ['geocode']});
}
...
function locateByAddress() {
  ...
  geocoder.geocode({address: location}, function(result, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      ...
      var placeLocation = geometry.location;
